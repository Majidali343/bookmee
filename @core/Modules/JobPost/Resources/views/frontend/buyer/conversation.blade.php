...
@section('site-title')
    {{__('Job Request Conversation Details')}}
@endsection
@section('style')
...
@section('content')
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="dashboard-settings margin-top-40">
                                <h2 class="dashboards-title"> {{__('Offer Details')}} </h2>
                            </div>
                            <x-msg.success/>
                            <x-msg.error/>
                        </div>
                                ...
                                <div class="card-body">
                                    <div class="gig-chat-message-heading">
                                        <div class="header-wrap d-flex justify-content-between">
                                            <h4 class="header-title">{{__('Job Offer Details')}}</h4>
                                            @if($request_details->is_hired === 1)
                                                <div>
                                                    <span class="btn btn-danger disabled">{{ __('Hired') }}</span>
                                                </div>
                                            @else
                                                <button class="btn btn-sm btn-info" data-toggle="modal" data-target="#addCouponModal">{{ __('Hire Now') }}</button>
                                            @endif
                                        </div>

                                        @if($request_details)
                                            <div class="gig-order-info">
                                                <p><strong>{{__('Request ID:') }}</strong> #{{ $request_details->id }}</p>
                                                <p><strong>{{__('Job Title:') }}</strong> {{ optional($request_details->job)->title }}</p>
                                                <p><strong>{{__('Cover Letter:') }}</strong> {{ $request_details->cover_letter }}</p>
                                                <p><strong>{{__('Offer Price:') }}</strong> {{ float_amount_with_currency_symbol($request_details->expected_salary) }}</p>
                                                 @php 
                                                $salary = $request_details->expected_salary;
                                                $user_details = auth("web")->user();
                                                //$user_details->country_id
                                                //optional($request_details->job)->country_id
                                                $tax =  App\Helpers\ServiceCalculationHelper::calculateTax($salary ,optional($request_details->job)->country_id);
                                                @endphp
                                                <p><strong>{{__('Tax:') }}</strong> {{  float_amount_with_currency_symbol( $tax) }}</p>
                                                <p><strong>{{__('Total:') }}</strong> {{ float_amount_with_currency_symbol($salary+$tax) }}</p>
                                                ...
                                                @forelse($all_messages as $msg)
                                                    <div class="single-message-item @if($msg->type == 'buyer') customer @endif">
                                                        <div class="top-part">
                                                            ...
                                                            <span class="title">
                                                                 @if($msg->type == 'buyer')
                                                                    {{substr(optional(optional($request_details->job)->buyer)->name ?? 'B',0,1)}}
                                                                @else
                                                                    {{substr(optional($request_details->seller)->name ?? 'S',0,1)}}
                                                                @endif
                                                            </span>
                                                                @if($msg->notify == 'on')
                                                            ...
                                                            <div class="content">
                                                                <h6 class="title">
                                                                    @if($msg->type == 'buyer')
                                                                        {{optional($request_details->seller)->name ?? 'S'}}

                                                                    @else
                                                                        {{optional(optional($request_details->job)->buyer)->name ?? 'B'}}
                                                                    @endif
                                                                </h6>
                                                        ...
                                                        <div class="content">
                                                            ...
                                                            @if(file_exists('assets/uploads/job-request/'.$msg->attachment))
                                                                <a href="{{asset('assets/uploads/job-request/'.$msg->attachment)}}" download class="anchor-btn">{{$msg->attachment}}</a>
                                                            @endif
                                                        </div>
                                                    </div>
                                                @empty
                                        ...
                                        <div class="reply-message-wrap ">
                                            ...
                                            <form action="{{route('buyer.job.request.message.send')}}" method="post" enctype="multipart/form-data">
                                                @csrf
                                                @if($request_details)
                                                    <input type="hidden" value="{{$request_details->id}}" name="request_id">
                                                @endif
    ...
    <div class="modal fade" id="addCouponModal" tabindex="-1" role="dialog" aria-labelledby="couponModal" aria-hidden="true">
        <form id="msform" class="ms-order-form" action="{{ route('buyer.job.request.seller.hired',$request_details->id) }}" method="post"  enctype="multipart/form-data">
            @csrf

            <input type="hidden" value="{{$request_details->id}}" name="request_id">
            <input type="hidden" value="{{ $request_details->job_post_id }}" name="job_post_id">
            <input type="hidden" value="{{ $request_details->expected_salary }}" name="expected_salary">
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary order_create_from_jobs">{{ __('Place Order') }}</button>
                    </div>
                </div>
            </div>
        </form>
        ...
        (function($){
            ...
            $(document).ready(function(){
                $(document).on('click','.swal_status_button',function(e){
                    e.preventDefault();
                    Swal.fire({
                        title: '{{__("Are you sure to hire this seller?")}}',
                        text: '{{__("You will not change it after hire!")}}',
                        icon: 'warning',
