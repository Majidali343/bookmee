...
@section('content')
                                ...
                                <div class="card-body">
                                    <div class="gig-chat-message-heading">
                                        ...
                                        @if($request_details)
                                            <div class="gig-order-info">
                                                ...
                                                <p><strong>{{__('Cover Letter:') }}</strong> {{ $request_details->cover_letter }}</p>
                                                 <p>
                                                     <strong>{{__('Offer Price:') }}</strong>
                                                     <b>{{ float_amount_with_currency_symbol($request_details->expected_salary) }}</b>

                                                @if($request_details->is_hired != 1)
                                                     <a href="#0" class="edit_offer_price"
                                                        data-toggle="modal"
                                                        data-target="#editOfferPrice"
                                                        data-id="{{ $request_details->id }}"
                                                        data-expected_salary="{{ $request_details->expected_salary }}"                                                     >
                                                         <span class="dash-icon dash-edit-icon color-1"> <i class="las la-edit btn btn-warning"></i> </span>
                                                     </a>
                                                @endif
                                        ...
                                        <div class="reply-message-wrap ">
                                            ...
                                            <form action="{{route('seller.job.request.message.send')}}" method="post" enctype="multipart/form-data">
                                                @csrf
                                                @if($request_details)
    ...
    <!-- Edit Job offer price -->
    <div class="modal fade" id="editOfferPrice" tabindex="-1" role="dialog" aria-labelledby="editOfferPrice" aria-hidden="true">
        <form action="{{ route('seller.job.offer.price.edit') }}" method="post">
            <input type="hidden" id="up_id" name="up_id" >
            @csrf
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editOfferPrice">{{ __('Edit Job Offer Price') }}</h5>
                    ...
                    <div class="modal-body">
                        <div class="form-group mt-3">
                            <label for="expected_salary">{{ __('Offer Price') }}</label>
                            <input type="text" name="expected_salary" id="expected_salary" class="form-control" placeholder="{{ __('Offer Price') }}">
                        </div>
                    </div>
    ...
    <script>
        (function($){
            ...
            $(document).ready(function(){
                ...
                // job offer price
                $(document).on('click','.edit_offer_price',function(e){
                    e.preventDefault();
                    let offer_id = $(this).data('id');
                    let expected_salary = $(this).data('expected_salary');
                    $('#up_id').val(offer_id);
