...
class SellerWalletPaymentController extends Controller
{
    protected function cancel_page()
    {
        return redirect()->route('seller.wallet.deposit.payment.cancel.static');
    }
    ...
    public function paypal_ipn_for_wallet(Request $request)
    {
        ...
        if (isset($payment_data['status']) && $payment_data['status'] === 'complete'){
            ...
            return redirect()->route('seller.wallet.history');
        }
    ...
    public function send_jobs_mail($last_deposit_id)
    {
        ...
        //Send email to admin
        try {
            $message_body = __('Hello a seller just deposit to his wallet.').'</br>'.'<span class="verify-code">'.__('Deposit ID: ').$last_deposit_id.'</span>';
