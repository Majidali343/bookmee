@section('scripts')
    ...
    <script
        src="//npmcdn.com/flatpickr/dist/l10n/{{ current(explode('_', \App\Helpers\LanguageHelper::user_lang_slug())) }}.js">
    </script>
            ...
            $(document).ready(function() {
                ...
                function extra_service_calculate() {
                    ...
                    for (let i = 0; i < additional_services.length; i++) {
                        ...
                        let unit_price = service_data.find($('.value-count')).text().replace(
                ...
                function subtotal_calculate() {
                    let package_fee = parseInt($(
                        'input[name="package_fee_input_hiddend_field_for_js_calculation"]').val());
                    let extra_service_fee = parseInt($('.extra-service-fee').text().replace(',', '').replace(
                        site_default_currency_symbol, ''));
                    let service_subtotal = package_fee + extra_service_fee;
                    $('.service-subtotal').text(site_default_currency_symbol + service_subtotal);
                    $('input[name="service_subtotal_input_hidden_field_for_js_calculation"]').val(
                        service_subtotal);
                }
                subtotal_calculate();
                ...
                function total_amount() {
                    tax_calculate()
                    let subtotal = $('input[name="service_subtotal_input_hidden_field_for_js_calculation"]')
                        .val();
                    let tax = parseFloat($('.tax-amount').text().replace(',', '').replace(
                        site_default_currency_symbol, ''));
                ...
                $('#choose_service_area').on('change', function() {
                    let area_text = $("#choose_service_area").val();
                ...
                $('.confirm-location .next').on('click', function() {
                    ...
                    let city_text = $('#choose_service_city').val();
                    let country_text = $('#choose_service_country').val();
                ...
                $(document).on('click', '.remove-service-list', function() {
                    ...
                    if (isNaN(quantity)) {
                        ...
                        for (let i = 0; i < cal_spans.length; i++) {
                            cal_spans[i].innerText = document.getElementsByClassName(
                                'inc_dec_include_service')[0].value;
                        }
                        $('input[name="package_fee_input_hiddend_field_for_js_calculation"]').val(
                            parseFloat(include_total_price));
                ...
                $(document).on('click', '.extra-services .check-input', function() {
                    ...
                    let unit_price = $('span[price=' + additional_service_id + ']').text().replace(
                        site_default_currency_symbol, '');
                    let quantity = $('#additional_service_quantity_' + additional_service_id).val();

                    if ($(this).is(":checked")) {
                        $('.extra-service-list').append(
                            '<div class="single-additional">\
                                                    <li class="list inc_dec_additional_service" id="additional_service_id_' +
                            additional_service_id + '">\
                                                        <span class="rooms">' + service_name + '</span>\
                                                        <span class="room-count service_quantity_count">' + quantity + '</span>\
                                                        <span class="value-count">' + site_default_currency_symbol +
                            unit_price + '</span>\
                                                    </li>\
                                                </div>');

                        $('.extra-service-list-2').append(
                            '<div class="single-additional-2">\
                                                    <li class="list inc_dec_additional_service additional_service_list" id="additional_service_id_2_' +
                            additional_service_id + '">\
                                                        <input type="hidden" class="additionalServiceID" value="' +
                            additional_service_id + '">\
                                                        <span class="rooms">' + service_name +
                            '</span>\
                                                        <span class="room-count additional_service_quantity service_quantity_count">' +
                            quantity + '</span>\
                                                        <span class="value-count">' + site_default_currency_symbol +
                            unit_price + '</span>\
                                                    </li>\
                        ...
                        $(".single-additional #additional_service_id_" + additional_service_id)
                            .remove();
                        $(".single-additional-2 #additional_service_id_2_" + additional_service_id)
                            .remove();
                        extra_service_calculate();
                        subtotal_calculate();
                        total_amount();
                ...
                $('.confirm-service .next').on('click', function() {
                    if (date_string_format == '' || available_schedule == '') {
                        Command: toastr["warning"]("{{ __('Please select date and schedule.!') }}",
                            "Aviso")
                        toastr.options = {
                            "closeButton": true,
                            "debug": false,
                ...
                $(document).on('change', '#service_available_dates', function() {
                    ...
                    let service_id = {{ $service_details_for_book->id }};
                        ...
                        success: function(res) {
                            ...
                            if (res.status == 'success') {
                                ...
                                if (all_lists) {
                                    ...
                                    $("#addingremove").removeClass('shedulescroll');
                                }
                ...
                $(document).on('submit', '.ms-order-form', function(e) {
                    ...
                    if (!$('.terms-and-conditions .check-input').is(":checked")) {
                        ...
                        Command: toastr["warning"](
                            "{{ __('Please agree with terms and conditions.!') }}", "Aviso")
                        toastr.options = {
                            "closeButton": true,
                            "debug": false,
                            "newestOnTop": false,
                    ...
                    if ($('input[name="selected_payment_gateway"]').val() == '') {
                        //error msg
                        Command: toastr["warning"]("{{ __('Please select payment gateway.!') }}",
                            "Aviso")
                        toastr.options = {
                            "closeButton": true,
                            "debug": false,
                    ...
                    for (let i = 0; i < included_services.length; i++) {
                        let include_service_quantity = $(included_services[i]).find(
                            '.include_service_quantity').text();
                        let include_service_id = $(included_services[i]).find('.includeServiceID')
                            .val();
                        services.push({
                        ...
                        $('#msform').append('<input type="hidden" name="services[' + i +
                            '][id]" value="' + include_service_id + '"/>');
                        $('#msform').append('<input type="hidden" name="services[' + i +
                            '][quantity]" value="' + include_service_quantity + '"/>');
                    }
                    ...
                    for (let i = 0; i < additional_services.length; i++) {
                        let additional_service_quantity = $(additional_services[i]).find(
                            '.additional_service_quantity').text();
                        let additional_service_id = $(additional_services[i]).find(
                            '.additionalServiceID').val();
                        additionals.push({
                            id: additional_service_id,
                            quantity: additional_service_quantity
                        })
                        $('#msform').append('<input type="hidden" name="additionals[' + i +
                            '][id]" value="' + additional_service_id + '"/>');
                        $('#msform').append('<input type="hidden" name="additionals[' + i +
                            '][quantity]" value="' + additional_service_quantity + '"/>');
                    }
                ...
                $(document).on('click', '.apply-coupon', function(e) {
                    e.preventDefault();
                    let total_amount = $('.total_amount_for_coupon').text().replace(',', '').replace(
                        site_default_currency_symbol, '');
                    let coupon_code = $('.coupon_code').val();
                    ...
                    $.ajax({
                        ...
                        data: {
                            ...
                            seller_id: seller_id,
                            service_id: {{ $service_details_for_book->id }},
                        },
                        success: function(res) {
                            if (res.status == 'success') {
                                let coupon_amount = res.coupon_amount;
                                let new_total = (total_amount - coupon_amount) * 1;
                                $('#total_amount_for_coupon').text(
                                    site_default_currency_symbol + new_total.toFixed(2));
                                $('.coupon_input_field').hide();
                                $('.coupon_amount_for_apply_code').html(
                                    '<strong>{{ __('Coupon Discount') }}</strong>' +
                                    site_default_currency_symbol + coupon_amount
                                    .toFixed(2))
                            }
                            if (res.status == 'invalid') {
                                Command: toastr["warning"](
                                    "{{ __('Coupon is invalid.!') }}", "Aviso")
                                toastr.options = {
                                    "closeButton": true,
                                    "debug": false,
                                    "newestOnTop": false,
                            ...
                            if (res.status == 'expired') {
                                Command: toastr["warning"](
                                    "{{ __('Coupon already expired.!') }}", "Aviso")
                                toastr.options = {
                                    "closeButton": true,
                                    "debug": false,
                                    "newestOnTop": false,
                            ...
                            if (res.status == 'error') {
                                Command: toastr["warning"](res.msg, "Aviso")
                                toastr.options = {
                                    "closeButton": true,
                                    "debug": false,
                                    "newestOnTop": false,
                            ...
                            if (res.status == 'notapplicable') {
                                Command: toastr["warning"](
                                    "{{ __('Coupon is not applicable for this service.!') }}",
                                    "Aviso")
                                toastr.options = {
                                    "closeButton": true,
                                    "debug": false,
                            ...
                            if (res.status == 'emptycoupon') {
                                Command: toastr["warning"](
                                    "{{ __('Please enter your coupon.!') }}", "Aviso")
                                toastr.options = {
                                    "closeButton": true,
                                    "debug": false,
                                    "newestOnTop": false,
