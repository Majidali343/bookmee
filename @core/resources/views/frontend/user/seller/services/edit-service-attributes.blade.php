                ...
                <div class="dashboard-right">
                    ...
                    <form action="{{ route('seller.edit.service.attribute', $service->id) }}" method="post">
                        ...
                        @if ($service->is_service_online == 1)
                            <input type="hidden" name="is_service_online_id" value="{{ $service->is_service_online }}"
                                id="is_service_online_id">
                        @endif
                        <div class="row">
                            <div class="col-xl-4 margin-top-50">
                                <div class="edit-service-wrappers">
                                    ...
                                    <div
                                        class="single-dashboard-input @if ($service->is_service_online == 1) service-price-show-hide @endif">
                                        <div class="single-info-input margin-top-50">
                                            <label class="info-title"> {{ __('Service Price') }}</label>
                                            <input class="form--control" type="text" name="price"
                                                id="service_total_price" value="{{ $service->price }}">
                                        </div>
                                    </div>

                                    <div class="btn-wrapper margin-top-40">
                                        <button type="submit" id="submit"
                                            class="cmn-btn btn-bg-1">{{ __('Update Attributes') }}</button>
                                    </div>
                                </div>
                            </div>
                            <div class="col-xl-8 margin-top-50">
                                    ...
                                    <div class="append-additional-includes">
                                        @foreach ($service_includes as $include)
                                            <div class="single-dashboard-input what-include-element">
                                                <input type="hidden" name="service_include_id[]"
                                                    value="{{ $include->id }}">
                                                <div class="single-info-input margin-top-20">
                                                    <label>{{ __('Title') }}</label>
                                                    <input class="form--control" type="text"
                                                        name="include_service_title[]"
                                                        placeholder="{{ __('Service title') }}"
                                                        value="{{ $include->include_service_title }}">
                                                </div>
                                                <div
                                                    class="single-info-input margin-top-20 @if ($service->is_service_online == 1) is_service_online_hide @endif">
                                                    <label>{{ __('Unit Price') }}</label>
                                                    <input class="form--control include-price" type="text"
                                                        name="include_service_price[]" placeholder="{{ __('Add Price') }}"
                                                        value="{{ $include->include_service_price }}">
                                                </div>
                                                <div
                                                    class="single-info-input margin-top-20 @if ($service->is_service_online == 1) is_service_online_hide @endif">
                                                    <label>{{ __('Quantity') }}</label>
                                                    <input class="form--control numeric-value" type="text"
                                                        name="include_service_quantity[]"
                                                        placeholder="{{ __('Add Quantity') }}"
                                                        value="{{ $include->include_service_quantity }}" readonly>
                                                </div>
                                            </div>
                                        @endforeach
                                    </div>
                                    ...
                                    <div class="append-additional-services">
                                        <div class="single-dashboard-input additional-services">
                                            <div class="serviceopt margin-top-20">
                                                ...
                                                <label>Time</label><br>
                                                <input type="number" id="points" name="time" step="15"
                                                    value={{ $service_time }} placeholder="Mins">
                                            </div>
                                            <div class="serviceopt margin-top-20">
                                                <h5 style="font-size: 20px;">Select Staff who can provide this Service</h5>
                                                <p>It can be all or any Specific</p><br>

                                                <button type="button" id="services_add_btn" data-toggle="modal"
                                                    data-target="#addstaff" data-services_selected="{{ __($data) }}">
                                ...
                                <input type="text" name="staffs" id="services_ids" value=""
                                    style="display: none;">
                                        ...
                                        <div class="single-dashboard-input">
                                            <div class="single-info-input margin-top-20">
                                                ...
                                                <input class="form--control" type="number"
                                                    value="{{ $service->delivery_days }}" step="0.01"
                                                    name="delivery_days" placeholder="{{ __('Delivery Days') }}">
                                            </div>
                                            <div class="single-info-input margin-top-20">
                                                <label>{{ __('Revisions') }}</label>
                                                <input class="form--control" type="number"
                                                    value="{{ $service->revision }}" step="0.01" name="revision"
                                    ...
                                    <div class="single-settings margin-top-40">
                                        ...
                                        <div class="append-faqs">
                                            @foreach ($online_service_faq as $faq)
                                                <div class="single-dashboard-input benifits">
                                                    <input type="hidden" name="online_service_faq_id[]"
                                                        value="{{ $faq->id }}">
                                                    <div class="single-info-input margin-top-20">
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 style="font-size: 16px;" class="modal-title" id="exampleModalLabel">Staff Member’s who can
                        provide This Service</h5>
                            ...
                            <li class="checkbox-item">
                                ...
                                @if (
                                    $data->profile_image_id !== null &&
                                        ...
                                        get_attachment_image_by_id($data->profile_image_id)['img_url'] !== '')
                                    <img class="modalimage"
                                        src="{{ get_attachment_image_by_id($data->profile_image_id)['img_url'] }}"
                                        alt="">
                                @else
    ...
    <script>
        (function($) {
            ...
            $(document).ready(function() {
                ...
                function handleServicesAddButtonClick() {
                    let services_selected = $('#services_add_btn').data('services_selected');
